...
function toIndexed(bufferGeometry: BufferGeometry) {
	...
	bufferGeometry.setAttribute('position', new BufferAttribute(positionBuffer, 3))
	if (hasNormal) bufferGeometry.setAttribute('normal', new BufferAttribute(normalBuffer!, 3))
	if (hasUV) bufferGeometry.setAttribute('uv', new BufferAttribute(uvBuffer!, 2))
